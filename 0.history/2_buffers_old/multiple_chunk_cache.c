...
#include "multiple_chunk_time.h"
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <string.h>
#include "../4_statistics/cache.h"
...
void *producer(void *arg) {
    ...
    struct timespec wait_start, exec_start;
    CachePerfCounter cache_counter;
    if (init_cache_counters(&cache_counter) == 0) {
        start_cache_counters(&cache_counter);
    }
    start_thread_timer(&producer->metrics);
    ...
    while (work_range->current_offset < work_range->end_offset) {
        ...
        stop_execution(&producer->metrics, &exec_start);
        start_waiting(&wait_start);
        ...
        stop_waiting(&producer->metrics, &wait_start);
    ...
    stop_thread_timer(&producer->metrics);
    finalize_metrics(&producer->metrics);
    pthread_exit(NULL);
}
...
void *consumer(void *arg) {
    ...
    struct timespec wait_start, exec_start;
    start_thread_timer(&consumer->metrics);
    ...
    while (1) {
        ...
        while (1) {
            ...
            stop_waiting(&consumer->metrics, &wait_start);
            ...
            if (buffer->full) {
                ...
                pthread_mutex_unlock(&buffer->lock);
                stop_execution(&consumer->metrics, &exec_start);
                break;
            }
        }
    ...
    stop_thread_timer(&consumer->metrics);
    finalize_metrics(&consumer->metrics);
