...
#include "../components/2_buffers/multiple_chunk_time.h"
...
int main(int argc, char *argv[]) {
    ...
    AggregatedMetrics prod_agg, cons_agg;
    init_aggregated_metrics(&prod_agg);
    init_aggregated_metrics(&cons_agg);

    int total_chunks = 0;
    for (int i = 0; i < Nprod; i++) {
        pthread_join(prod[i], NULL);
        total_chunks += producers[i].handledChunks;
        add_thread_metrics(&prod_agg, &producers[i].metrics);
        destroy_thread_metrics(&producers[i].metrics);
        free(producers[i].work_range);
    }
    calculate_averages(&prod_agg);
    ...
    for (int i = 0; i < Ncons; i++) {
        ...
        add_thread_metrics(&cons_agg, &consumers[i].metrics);
        destroy_thread_metrics(&consumers[i].metrics);
        free(consumers[i].stats);
    }
    calculate_averages(&cons_agg);
    ...
    print_aggregated_metrics("PRODUCER", &prod_agg);
    print_aggregated_metrics("CONSUMER", &cons_agg);
    return 0;
}